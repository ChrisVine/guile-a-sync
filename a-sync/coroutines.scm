...
;; same waitable procedure call still runs, the exception will
;; re-propagate out of 'resume' in that callback, where it will be
;; uncatchable, and then out of the event loop - if the event loop in
;; the event-loop module is used, this means that it will propagate
;; out of the call to event-loop-run!.  It is therefore best if such
;; exceptions are handled locally within the waitable procedure.  Any
;; exception thrown in the waitable procedure after the first call to
...
;; returned to the event loop and the remainder of the waitable
;; procedure will not execute.  If that exception is not handled
;; locally in the callback, or if the callback throws an exception of
;; its own, then it will propagate out of the event loop - if the
;; event loop in the event-loop module is used, this means that it
;; will propagate out of the call to event-loop-run!.  If an exception
;; propagates out of event-loop-run! for that or some other reason,
;; then the event loop will be left in a valid state and it will be as
;; if event-loop-quit! had been called on it, but it is then up to the
...
;; which will correctly set this up for you automatically.  If those
