...
;; 'meeting' object.  From version 0.16, multiple senders may wait on
...
(define meeting-send
  (case-lambda 
    ...
    ((await resume loop m datum)
	 ...
	 ((set)
	  (when (q-empty? res)
	    (error "meeting-send encountered a set meeting object with no sender resumption iterator"))
	  (enq! res (cons resume datum))
	  (await))
	 ((unset)
	  (if (q-empty? res)
	      (begin
...
;; same 'meeting' object.  From version 0.16, multiple receivers may
