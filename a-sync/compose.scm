...
;; await-generator-in-event-loop!, await-timeout!, await-getline!,
...
;; event-loop) module are 'compose-a-sync'-capable.  In addition, to
;; make an ordinary body of code which does not block (and which does
;; not need to invoke a-sync's await procedure) usable by
