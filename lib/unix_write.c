...
/*
  ...
  This procedure returns the number of bytes actually written, which
  can be less than 'count' bytes.  The sum of 'begin' and 'count' must
  ...
  EAGAIN, EWOULDBLOCK or EINTR, a 'c-write-error exception is thrown
  and errno is given as an argument to the exception handler.  EINTR
  is handled internally and is not an error.
